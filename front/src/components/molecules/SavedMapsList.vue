...
<template>
  <ul v-if="maps.length > 0" class="flex flex-col gap-1">
    <li
      ...
      :class="m.id === activeId ? 'bg-cyan-500/20' : 'bg-surface'"
    >
      <button type="button" class="flex-1 text-left" @click="$emit('load', m.id)">
        <span class="font-semibold">{{ m.name }}</span>
        <span class="text-ink/40"> · {{ m.width }}×{{ m.height }}</span>
      </button>
      <button
        type="button"
        aria-label="Exporter"
  ...
  <p v-else class="text-xs text-ink/40">Aucune carte enregistrée pour l'instant.</p>
</template>
